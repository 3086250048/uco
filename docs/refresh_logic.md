...
`find-index` 表示“重建按 IP 查询索引”。

它会执行：

```bash
/root/work/bin/csv_to_ip.py
```

作用是把已有采集结果整合成 `/srv/smb/find/<IP>.txt` 文件，方便在共享目录里按 IP 查询信息。

主要整合的数据来源：

| 数据目录 | 内容 |
| --- | --- |
| `/srv/smb/mac_table/` | MAC/ARP、有线接入口信息 |
| `/srv/smb/Wireless_user/` | 无线用户、AP、APID、MAC、IP |
| `/srv/smb/icg_users/` | 已有缓存的 ICG 应用控制策略信息 |

输出文件示例：

```text
/srv/smb/find/10.20.59.209.txt
/srv/smb/find/10.20.61.120.txt
```

注意：`find-index` 不会实时查询交换机或 ICG。它只是把已经采集到的数据和已有 ICG 缓存重新整理成按 IP 查询的文本索引。
